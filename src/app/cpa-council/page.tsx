...
export default function CPACouncilPage() {
  ...
  return (
    <div style={{ backgroundColor: '#faf8f4', minHeight: '100vh' }}>
      ...
      <nav style={{ backgroundColor: '#fff', borderBottom: '1px solid #e8e0d4', padding: '0 32px', position: 'sticky', top: 0, zIndex: 40 }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            <Link href="/certification" style={{ fontSize: 14, color: '#6b6560', textDecoration: 'none' }}>Certification</Link>
      ...
      {/* Stats bar */}
      <div style={{ backgroundColor: '#1a1714', padding: '20px 32px' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '80px 32px' }}>
        ...
        <div style={{ marginBottom: 80 }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: '#1a1714', letterSpacing: '-0.02em' }}>What Council Members Get</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
            {BENEFITS.map((b, i) => (
              <div key={i} style={{ backgroundColor: '#fff', border: '1px solid #e8e0d4', borderRadius: 14, padding: 28 }}>
                ...
                <h3 style={{ fontSize: 16, fontWeight: 700, color: '#1a1714', marginBottom: 8 }}>{b.title}</h3>
                <p style={{ fontSize: 14, color: '#6b6560', lineHeight: 1.6, margin: 0 }}>{b.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 80, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'center' }}>
          <div>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: '#1a1714', letterSpacing: '-0.02em', marginBottom: 20 }}>Who We&apos;re Looking For</h2>
            <p style={{ fontSize: 15, color: '#6b6560', lineHeight: 1.7, marginBottom: 28 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {CRITERIA.map((c, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  <div style={{ width: 22, height: 22, borderRadius: '50%', backgroundColor: '#2d5a27', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
                    <span style={{ color: '#fff', fontSize: 11, fontWeight: 700 }}>✓</span>
                  </div>
                  <span style={{ fontSize: 14, color: '#1a1714', lineHeight: 1.5 }}>{c}</span>
                </div>
              ))}
            </div>
          </div>
          <div style={{ backgroundColor: '#fff', border: '1px solid #e8e0d4', borderRadius: 16, padding: 36 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#1a1714', marginBottom: 20 }}>The Opportunity</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i < 4 ? '1px solid #e8e0d4' : 'none' }}>
                  <span style={{ fontSize: 13, color: '#6b6560' }}>{row.label}</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: '#1a1714' }}>{row.value}</span>
                </div>
              ))}
            </div>
            <div style={{ marginTop: 16, padding: '12px 16px', backgroundColor: '#fdf2e9', borderRadius: 10 }}>
        ...
        <div id="members" style={{ marginBottom: 80 }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: '#1a1714', letterSpacing: '-0.02em' }}>Current Council Members</h2>
            <p style={{ fontSize: 15, color: '#6b6560', marginTop: 10 }}>27 of 50 seats filled · Applications close Dec 31, 2024</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
            {COUNCIL_MEMBERS.map((m, i) => (
              <div key={i} style={{ backgroundColor: '#fff', border: '1px solid #e8e0d4', borderRadius: 14, padding: 24, display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                <div style={{
                ...
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#1a1714', marginBottom: 2 }}>{m.name}</div>
                  <div style={{ fontSize: 13, color: '#6b6560' }}>{m.role}</div>
                  <div style={{ fontSize: 12, color: '#a09a94' }}>{m.firm}</div>
                  <div style={{ marginTop: 8, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 10, backgroundColor: '#fdf2e9', color: '#92400e', fontWeight: 600 }}>{m.location}</span>
                    <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 10, backgroundColor: '#f0fdf4', color: '#2d5a27', fontWeight: 600 }}>{m.years} yrs exp</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div id="apply" style={{ maxWidth: 680, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: '#1a1714', letterSpacing: '-0.02em' }}>Apply to Join the Council</h2>
            <p style={{ fontSize: 15, color: '#6b6560', marginTop: 10 }}>Applications are reviewed within 48 hours. We accept approximately 2-3 new members per week.</p>
          </div>

          {submitted ? (
            <div style={{ backgroundColor: '#fff', border: '2px solid #2d5a27', borderRadius: 16, padding: 48, textAlign: 'center' }}>
              <div style={{ fontSize: 48, marginBottom: 16 }}>✓</div>
              <h3 style={{ fontSize: 22, fontWeight: 700, color: '#2d5a27', marginBottom: 8 }}>Application Submitted!</h3>
            ...
            <form onSubmit={handleSubmit} style={{ backgroundColor: '#fff', border: '1px solid #e8e0d4', borderRadius: 16, padding: 40 }}>
              ...
              <div style={{ marginBottom: 24 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: '#1a1714', display: 'block', marginBottom: 6 }}>
                ...
                <textarea
                  ...
                  required
                  rows={5}
                  style={{ width: '100%', padding: '12px 14px', borderRadius: 8, border: '1px solid #e8e0d4', fontSize: 14, color: '#1a1714', backgroundColor: '#faf8f4', resize: 'vertical', boxSizing: 'border-box' }}
                />
              </div>
              ...
              <button
                ...
                style={{
                  ...
                  backgroundColor: loading ? '#a09a94' : '#b8734a', color: '#fff',
      ...
      {/* Footer */}
      <div style={{ borderTop: '1px solid #e8e0d4', padding: '32px', textAlign: 'center' }}>
        <p style={{ fontSize: 13, color: '#a09a94' }}>© 2024 CloseBooks, Inc. · <Link href="/dashboard" style={{ color: '#b8734a', textDecoration: 'none' }}>Sign In</Link> · <Link href="/certification" style={{ color: '#b8734a', textDecoration: 'none' }}>Certification</Link></p>
      </div>
    </div>
  )
}
